...
# Backup 21:04 608 date
# Back up, 17:38, 1008, no interpolation sol for limitation 2.
...
# Decay is defined, let's go irf
from scipy.interpolate import interp1d

# --------------------------------------------------
# Load IRF & Resample to Match Measurement Time Base
# --------------------------------------------------

irf_data = load_hipic_prf("triggering_1ns_vertical.prf")
irf_time = irf_data[:, 0]
irf_counts_raw = irf_data[:, 1]

# 1. Maak een interpolatiefunctie van de originele IRF
# (We gebruiken bounds_error=False en fill_value=0 voor punten buiten het bereik)
irf_interpolator = interp1d(irf_time, irf_counts_raw, kind='linear', bounds_error=False, fill_value=0.0)

# 2. Bepaal de tijdsas van je meetdata in nanoseconds (of dezelfde eenheid als irf_time)
# Stel dat je meetdata een lengte heeft van bijv. len(decay) pixels:
t_measurement_ns = np.arange(len(decay)) * DT_NS

# 3. Genereer een nieuwe gesampled IRF-reeks op exact dezelfde tijdbasis als je metingen!
irf_counts = irf_interpolator(t_measurement_ns)

# Normaliseer de geïnterpoleerde IRF opnieuw
irf_counts = irf_counts / np.sum(irf_counts)

# Nadat je de IRF hebt ingelezen en genormaliseerd:
# Snijd eventuele achtergrond/ruis aan het uiteinde van de IRF weg of maak het nul
irf_peak_idx = np.argmax(irf_counts)
# Alles ver voorbij de piek (waar de laserpuls allang gedoofd is) mag hard op 0 gezet worden:
irf_counts[irf_peak_idx + 150:] = 0.0
...
# --------------------------------------------------
# Mono + IRF Fit (volledig in pixel-eenheden)
# --------------------------------------------------
popt_irf, pcov_irf = curve_fit(
    lambda t, A, tau_pixels, t0: irf_mono_decay_causal(
        t, A, tau_pixels, t0, irf_counts
    ),
    t_fit_irf,
    y_fit_irf,
    sigma=sigma_fit_irf,
    ...
    p0=[
        np.max(y_fit_irf),
        2.5 / DT_NS,   # Reken een typische levensduur van ~2.5 ns om naar pixels! (tau_ns / DT_NS)
        rel_peak_D     # Start-offset in pixels (waar de piek zit)
    ],
    bounds=(
        [0, 0.1 / DT_NS, -10],             # Ondergrenzen in pixels
        [np.inf, 50.0 / DT_NS, len(t_fit_irf)] # Bovengrenzen in pixels
    ),
    maxfev=50000
)
...
# --------------------------------------------------
# Mono + IRF + BG Fit (volledig in pixel-eenheden)
# --------------------------------------------------
popt_irf_bg, pcov_irf_bg = curve_fit(
    lambda t_arr, A, tau_pixels, t0, B: irf_mono_decay_causal_bg(
        t_arr, A, tau_pixels, t0, B, irf_counts
    ),
    t_fit_irf,
    y_fit_irf,
    sigma=sigma_fit_irf,
    ...
    p0=[
        A_guess_D,
        2.5 / DT_NS,   # Ook hier tau omgerekend naar pixels
        rel_peak_D,
        initial_bg_D
    ],
    bounds=(
        [0, 0.1 / DT_NS, -10, 0],             # Ondergrenzen
        [np.inf, 50.0 / DT_NS, len(t_fit_irf), np.max(y_fit_irf)] # Bovengrenzen
    ),
    maxfev=50000
)
# Back up 19:54, 10/08, we are so doing this, we got this.
...
time_fit_irf_ns_D = (t_fit_irf + t[fit_start_irf]) * DT_NS
...
for file in tif_files:
    ...
    popt_irf, pcov_irf = curve_fit(
        lambda t,A,tau,t0:
         ...
         bounds=(
             [0, 0.1 / DT_NS, -10],      # Ondergrenzen: A >= 0, tau >= 0, t0 mag eventueel negatief zijn (afhankelijk van je tijdsschaal)
             [np.inf, 50.0 / DT_NS, len(t_fit_irf)] # Bovengrenzen
         ),
         maxfev=50000
    )
    ...
    A_guess_DA = 1.2 * (peak_val_sample - initial_bg_sample)
    rel_peak_DA = peak_index
    ...
    popt_irf_bg, pcov_irf_bg = curve_fit(
        ...
        p0=[A_guess_DA, 2 / DT_NS, rel_peak_DA, initial_bg_sample],
        bounds=([0, 0.1 / DT_NS, -10, 0], [np.inf, 50.0 / DT_NS, len(t_fit_irf), np.max(y_fit_irf)] ),
        maxfev=50000 
    ) # Yes, we are here.
